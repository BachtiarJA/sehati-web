...
export default function Diagnosis({ pemeriksaans = [], antrianAktif = [] }: Props) {
    ...
    const handleDelete = (id: number) => {
        if (window.confirm('Hapus data pemeriksaan ini?')) {
    ...
    const exportToExcel = () => {
        ...
        XLSX.writeFile(wb, `Laporan_Pemeriksaan_${new Date().getTime()}.xlsx`);
    };
                    ...
                    <div className="flex w-full flex-col items-center gap-3 sm:w-auto sm:flex-row">
                        <div className="relative w-full sm:w-auto">
                            ...
                            <input
                                ...
                                className="w-full rounded-xl border border-slate-200 bg-white py-2 pr-4 pl-10 text-sm shadow-sm focus:border-emerald-500 focus:ring-4 focus:ring-emerald-500/10 focus:outline-none sm:w-64"
                            />
                        </div>
                        <div className="relative w-full sm:w-auto">
                            <Calendar className="absolute top-1/2 left-3.5 -translate-y-1/2 text-slate-400" size={18} />
                            <select
                                value={filterWaktu}
                                onChange={(e) => {
                                    setFilterWaktu(e.target.value as any);
                                    if (e.target.value !== 'tanggal_tertentu') setTanggalSpesifik('');
                                }}
                                className="w-full cursor-pointer appearance-none rounded-xl border border-slate-200 bg-white py-2 pr-8 pl-10 text-sm font-medium shadow-sm focus:border-emerald-500 focus:ring-4 focus:ring-emerald-500/10 focus:outline-none sm:w-44"
                            >
                        ...
                        {filterWaktu === 'tanggal_tertentu' && (
                            <input
                                ...
                                className="w-full rounded-xl border border-slate-200 bg-white px-4 py-2 text-sm shadow-sm focus:border-emerald-500 focus:ring-4 focus:ring-emerald-500/10 focus:outline-none sm:w-auto"
                            />
                        )}
                    </div>
                        ...
                        <table className="w-full border-collapse text-left">
                            <thead>
                                <tr className="border-b border-slate-200 bg-slate-50/80">
                                    ...
                                    <th className="px-6 py-4 text-xs font-bold tracking-wider text-slate-500 uppercase">TB/BB</th>
                                    ...
                                    <th className="w-1/5 px-6 py-4 text-xs font-bold tracking-wider text-slate-500 uppercase">Tindakan</th>
                            ...
                            <tbody className="divide-y divide-slate-100">
                                {filteredData.map((item) => (
                                    <tr key={item.id} className="group transition-colors hover:bg-slate-50/50">
                                        ...
                                        <td className="px-6 py-4 text-sm text-slate-600">{item.keluhan}</td>
                                ...
                                <div className="space-y-1.5">
                                    ...
                                    <div className="relative">
                                        <input
                                            ...
                                            className="w-full rounded-xl border border-slate-200 bg-white px-4 py-2.5 pr-10 text-sm text-slate-800 transition-all focus:border-emerald-500 focus:ring-4 focus:ring-emerald-500/10 focus:outline-none"
                                            placeholder="0"
                                        />
                                        <span className="absolute top-1/2 right-4 -translate-y-1/2 text-sm font-medium text-slate-400">cm</span>
                                    </div>
                                </div>
                                <div className="space-y-1.5">
                                    <label className="text-sm font-medium text-slate-700">Berat Badan</label>
                                    <div className="relative">
                                        <input
                                            type="number"
                                            required
                                            ...
                                            placeholder="0"
                                        />
                                        <span className="absolute top-1/2 right-4 -translate-y-1/2 text-sm font-medium text-slate-400">kg</span>
                                    </div>
                                </div>
                            </div>
